...
process_chains <- function(datafile, burnin) {
	...
	if (nrow(chain) > 1 ) {
	    print (sprintf("                             merging %s (%d rows)", datafile, nrow(chain)))
...
print (sprintf("                                     mean score = %g", mean(data[,opt$score])))
print (sprintf("                                     Rows read = %d", nrow(data)))
#print (sprintf("                                     parameters = %g", dataset))
...
print (sprintf("Printing parameter distributions for %s", opt$input))
...
print (sprintf("Printing parameter distribution densities for %s", opt$input))
...
print (sprintf("Printing parameter chains for %s", opt$input))
...
print (sprintf("Printing convergence for %s", opt$input))
...
print (sprintf("Printing chain distribution %s", opt$input))
...
if (opt$gelman) {
	...
	print (sprintf("Gelman-Rubin convergence statistics for %s", opt$input))
	...
	print(paste("    Could not produce Gelman-Rubin plot:  ",err))
    })
...
print (sprintf("Printing parameter correlations %s", opt$input))
